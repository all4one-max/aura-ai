...
def rank_merged_images(
    ...
    for idx, img_vec in enumerate(merged_image_embeddings):
        ...
        style_emb = np.array(user_embedding.style_embedding) if isinstance(user_embedding.style_embedding, list) else user_embedding.style_embedding
        brand_emb = np.array(user_embedding.brand_embedding) if isinstance(user_embedding.brand_embedding, list) else user_embedding.brand_embedding
        color_emb = np.array(user_embedding.color_embedding) if isinstance(user_embedding.color_embedding, list) else user_embedding.color_embedding
        intent_emb = np.array(user_embedding.intent_embedding) if isinstance(user_embedding.intent_embedding, list) else user_embedding.intent_embedding
...
def ranking_agent(
    ...
    beauty_standard_embedding = config.get("metadata", {}).get("beauty_standard_embedding")
