...
async def search_google_shopping(query_filters: Dict[str, Any]) -> List[Product]:
    ...
    try:
        ...
        if skipped_count > 0:
            print(
                f"⚠️ Skipped {skipped_count} product(s) due to missing required fields."
            )
        ...
        if filter_stats["price_filtered"] > 0:
            print(
                f"⚠️ Filtered out {filter_stats['price_filtered']} product(s) outside price range."
            )

        if filter_stats["rating_filtered"] > 0:
            print(
                f"⚠️ Filtered out {filter_stats['rating_filtered']} product(s) below minimum rating."
            )
